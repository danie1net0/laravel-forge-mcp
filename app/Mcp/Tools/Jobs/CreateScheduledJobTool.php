...
class CreateScheduledJobTool extends Tool
{
    ...
    public function handle(Request $request, ForgeService $forge): Response
    {
        ...
        if ($frequency === 'custom') {
            $data['minute'] = $request->string('minute', '*');
            $data['hour'] = $request->string('hour', '*');
            $data['day'] = $request->string('day', '*');
            $data['month'] = $request->string('month', '*');
            $data['weekday'] = $request->string('weekday', '*');
        }

        try {
            $job = $forge->createJob($serverId, $data);
